    ...
    <main>
          <?php {
                if(isset($_POST['confirm'])) 
                 {$id = $_GET['id'];
                                  $sql="UPDATE offres SET statut='Approuvé' WHERE id='$id' ";
                                  $res = $conn->query($sql); 
                 } 
                } 
          ?>
                ...
                <form method="post" action="">
                  <ul class="list-unstyled">
                    <div class="mb-3" style="display: flex;">
                      ...
                      <div class="col-lg-6">
                        <li>
                          <a href="<?php echo "profilutili.php?id=".$uid;?>" > <?php echo getNom($uid)." ".getPrenom($uid); ?></a>
                        </li>
                      </div>
                  ...
                  <div class="mt-3 mb-3" style="display: flex;">
                    ...
                    <div class="col-lg-6 mx-5">
                      <?php  if($statut=='Approuvé') 
                        { ?>  
                          <button class="btn btn-success" name="confirm" id="confirmer" onclick="confirmer()" disabled>Confirmer</button> 
                          <a  onclick="return confirm('Êtes vous sûr de vouloir supprimer ce trajet ?')"  href="supprimertrajet.php?Supprimer=<?php echo "".$id  ;?>" class="btn btn-danger" >Supprimer</a>

                          <a href="modifiertrajet.php?modifier=<?php echo $row['id'];?>" class="btn btn-primary"  onclick="return confirm('Êtes vous sûr de vouloir modifier ce trajet ?')" name="edit">Modifier</a>                       
                                <?php 
                         } 
                           elseif(($statut=='Supprimé'))
                         { 
                        ?>
                        <span class="bg-dark text-light">Trajet Supprimé par l'automobiliste</span>
                        <a  onclick="return confirm('Êtes vous sûr de vouloir supprimer ce trajet ?')"  href="supprimertrajet.php?Supprimer=<?php echo "".$id  ;?>" class="btn btn-danger" >Supprimer</a>
                        <?php 
                         }
                         ...
                         {
                        ?>
                        <span class="bg-warning text-body">Trajet refusé par l'admin</span>
                        <a  onclick="return confirm('Êtes vous sûr de vouloir supprimer ce trajet?')"  href="supprimertrajet.php?Supprimer=<?php echo "".$id  ;?>" class="btn btn-danger" >Supprimer</a>
                        <?php 
                         } else 
                         { 
                        ?> 
                            <button class="btn btn-success" name="confirm" id="confirmer" onclick="confirmer()" >Confirmer</button>
                        <?php 
                         } 
                        ?>      
                        <?php  if($statut=='Supprimer') 
                         { 
                        ?>                        
                            <a  onclick="return confirm('Êtes vous sûr de vouloir supprimer ce trajet ?')"  href="supprimertrajet.php?Supprimer=<?php echo "".$id  ;?>" class="btn btn-danger" disabled>Supprimer</a>
                        ...
                        <?php  if($statut=='En attente') 
                         { 
                        ?>  
                            <a  onclick="return confirm('Êtes vous sûr de vouloir refuser ce trajet ?')"  href="refusétrajet.php?Refuser=<?php echo "".$id  ;?>" class="btn btn-warning" >Refuser</a> 
                            <a  onclick="return confirm('Êtes vous sûr de vouloir supprimer ce trajet ?')"  href="supprimertrajet.php?Supprimer=<?php echo "".$id  ;?>" class="btn btn-danger" >Supprimer</a> 
                        <?php 
                         }
                        ?> 
